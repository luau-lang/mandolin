...
Mandolin surfaces lint violations reported by `lute lint`, and supports specifying custom lint rules on a per-project basis. Mandolin also enables VSCode debugging via the Lute debugger. For more information on how to use the Lute debugger, see [this reference](https://lute.luau.org/cli/debug/#connecting-to-the-vs-code-mandolin-extension).
...
- `mandolin.luteExecPath`: A path pointing to the `lute` binary to use to generate lint warnings or to run `lute debug serve` for the debugger.
...
`.vscode/extensions.json` lists a number of recommended VS Code extensions, which will let you do a number of useful things like running and debugging Mandolin and its tests from the command palette.
